function BER = conv_gen_plot_soft()
    ...
    for dB = 0:0.25:5
        ...
        rec_data_soft = conv_dec_soft(received_symbols_with_noise, g);
        ber = vpa(sum(rec_data~=sequence)/length(sequence))
        BER_soft(1,i) = vpa(max(sum(rec_data_soft~=sequence)/length(sequence), 10^-6))
        BER(1,i) = vpa(ber, 6)
        i=i+1
    end
