function decoded_data = conv_dec_soft(encoded_data, impulse_response)
    ...
    for i = 1:num_input_bits
        ...
        for j = 1:num_paths
            ...
            for k = 0:1
                ...
                % transition_bits
                bpsk_symbols = sqrt(E0)*(-1).^transition_bits;
                % received_symbols
                % hamming_distance = sum(mod(transition_bits' + received_symbols, 2));
                % hamming_distance
                euclidean_distance = sum((bpsk_symbols' - received_symbols).^2);
                new_metric = metrics(j, i) + euclidean_distance;
                % new_metric
